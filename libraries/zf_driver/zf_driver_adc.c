...
// 参数说明     count           转换次数
// 返回参数     void
// 使用示例     adc_mean_filter(ADC1_CH0_P10, 5);  //采集A0端口返回8位分辨率的AD值，采集五次取平均值
//-------------------------------------------------------------------------------------------------------------------
...
//  @return     void
//  Sample usage:               adc_convert(ADC1_CH0_P10);
//-------------------------------------------------------------------------------------------------------------------
uint16 adc_convert(adc_channel_enum ch)
{
    ...
    if((ch >> 12 )== 0)
    {
        ...
        adc_value = ADC_RES << 8;  			// 存储 ADC 的 12 位结果的高 4 位
        adc_value |= ADC_RESL;  		    // 存储 ADC 的 12 位结果的低 8 位
        
        ADC_RES = 0;
        ADC_RESL = 0;
        
        adc_value >>= adc_resolution[0];	// 取多少位
    }
    else if((ch >> 12) == 1)
    {
        ADC2_CONTR &= ~0x20;  			    // 清完成标志
        ADC2_CONTR &= (0xF0);			    // 清除ADC_CHS[3:0] ： ADC 模拟通道选择位
        ADC2_CONTR |= (ch >> 8) & 0xF;  
            
        ADC2_CONTR |= 0x40;  			    // 启动 AD 转换
            
        while (!(ADC2_CONTR & 0x20));  	    // 查询 ADC 完成标志
            
        adc_value = ADC2_RES << 8;  		// 存储 ADC 的 12 位结果的高 4 位
        adc_value |= ADC2_RESL;  		    // 存储 ADC 的 12 位结果的低 8 位
        
        ADC2_RES = 0;
        ADC2_RESL = 0;
        
        adc_value >>= adc_resolution[1];	//取多少位
...
// 返回参数     void
// 使用示例     adc_init(ADC1_CH0_P10, ADC_12BIT);//初始化P10引脚为ADC功能，12位分辨率
//-------------------------------------------------------------------------------------------------------------------
void adc_init(adc_channel_enum ch, adc_resolution_enum resolution)
{
    gpio_init(ch & 0xFF, GPI, 1, GPI_ANALOG);
    ...
    if((ch >> 12) == 0)
    {
        ...
        ADCCFG |= ADC_SYSclk_DIV_32;	        // ADC时钟频率SYSclk/16&0x0F;
        ADCCFG |= 1 << 5;					    // 转换结果右对齐。 ADC_RES 保存结果的高 2 位， ADC_RESL 保存结果的低 8 位。
        
        adc_resolution[0] = resolution;         // 记录ADC精度 将在采集时使用
    ...
    else if((ch >> 12) == 1)
    {
        ...
        ADC2CFG |= ADC_SYSclk_DIV_32;           // ADC时钟频率SYSclk/16&0x0F;
        ADC2CFG |= 1 << 5;			            // 转换结果右对齐。 ADC_RES 保存结果的高 2 位， ADC_RESL 保存结果的低 8 位。
        
        adc_resolution[1] = resolution;         // 记录ADC精度 将在采集时使用

        ADC2_CONTR |= 1 << 7;				    // 打开 ADC 电源
    }
    system_delay_ms(1);
}